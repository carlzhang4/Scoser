...
Blockly.Go.chaincode_body = function(block) {
	...
	code += '    if function == "init" {\n';
	code += '      fmt.Printf("Function is init")\n';
	code += '      return t.Init(stub, function, args)\n';
	code += '    }';
	
	for(var funcStream= block.getChildren()[0];funcStream; funcStream = funcStream.getNextBlock()){
		funcName = funcStream.type.substring(10);
		addinvoke();
	}
	code += '\n';
	...
	return code + '\n';
	function addinvoke(){
		code += ' else if function == "'+ funcName +'" {\n';
		code += '      fmt.Printf("Function is ' + funcName + '")\n';
		code += '      return t.' + funcName + '(stub, args)\n';
		code += '    }';
		return ;
	}
